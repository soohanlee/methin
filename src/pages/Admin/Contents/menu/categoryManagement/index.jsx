...
function CategoryManagementPage(props) {
  ...
  let initLinearData = useMemo(() => {
    return makeTreeToLinear(initData.current);
  }, [initData.current]);
  ...
  function setMenuItemTitle(item) {
    item.title = <MenuItem id={item.key} title={item.name}></MenuItem>;
  }
        ...
        <Space>
          <Form name="menu" onFinish={addMenu} layout="inline">
            <Form.Item
              ...
              name="name"
              rules={[
                { required: true, message: '메뉴 이름은 필수입니다!' },
                { max: 20, message: '최대 길이는 20자입니다.' },
              ]}
            >
              <Input />
            </Form.Item>
